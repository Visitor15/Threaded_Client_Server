	...
	public void executeTask() {
		...
		do {
			...
			try {
				...
				//
				// System.out.println("Wrote " + task.toBytes().length +
				// " bytes");
				//
				// DataOutputStream dataOutStream = new
				// DataOutputStream(m_SendingSocket.getOutputStream());
				//
				...
				ByteArrayInputStream arrayInStream = new ByteArrayInputStream(
						bytes);
				BufferedInputStream bis = new BufferedInputStream(arrayInStream);
				BufferedOutputStream out = new BufferedOutputStream(
						m_SendingSocket.getOutputStream());
